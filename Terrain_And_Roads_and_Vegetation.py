...
def generate_roads(terrain_map, density=0.2, highway_spacing=50):
    ...
    for i in range(0, width, int(highway_spacing)):
        for j in range(0, height, int(highway_spacing)):
            if random.random() < density:
                ...
                # Draw the road segment
                draw.line([(start_x, start_y), (start_x, end_y)], fill=(165, 160, 140), width=3)
                draw.line([(start_x, end_y), (end_x, end_y)], fill=(165, 160, 140), width=3)
